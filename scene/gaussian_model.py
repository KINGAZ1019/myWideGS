...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        self.network_fn = EMFeatureNetwork().cuda()
    ...
    def restore(self, model_args, training_args):
        ...
        self.network_fn.load_state_dict(network_fn_dict)
        self.optimizer2.load_state_dict(opt_dict2)
    ...
    def fetchPly(self, ply_path):
        ...
        colors = np.abs(positions) / np.max(np.abs(positions))
        # normals = np.vstack([vertices['nx'], vertices['ny'], vertices['nz']]).T
        return BasicPointCloud(points=positions, colors=colors)
    
    def gaussian_init(self, dataset_path):

        ply_path = os.path.join(dataset_path, "point_cloud.ply")
    ...
    def dist_signal_mapping(self, tx, r_o, freq):
        r_o = torch.tensor(r_o).cuda()
        pts = self._xyz
        view = self._xyz - tx
        tx = tx.repeat(pts.size(0), 1)
        freq = freq.repeat(pts.size(0), 1)
        raw = self.network_fn(pts, tx, view, freq)
...
class EMFeatureNetwork(nn.Module):
    ...
    def __init__(self, D=8, W=128, skips=[1,3,5,7], # skips 定义了在哪些层“之后”进行残差连接
        ...
        # MLP 的第 0 层：输入是经过频率调制后的 pts (维度 self.input_pts_dim)，输出是 W
        self.attenuation_linears.append(nn.Linear(self.input_pts_dim, W)) 
        
        # MLP 的后续 D-1 层：每一层的输入和输出维度都是 W。
        # 残差连接是元素级相加，不改变线性层的输入维度。
        for i in range(D - 1): # i 从 0 到 D-2，代表要构建的层的索引 (即 MLP 的第 1 到 D-1 层)
    ...
    def forward(self, pts, view, tx, freq):
        ...
        # 注意：这里的循环是针对 self.attenuation_linears 中的每个线性层
        # layer_input 变量用来保存当前线性层的输入，以便做残差连接
