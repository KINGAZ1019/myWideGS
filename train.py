...
def training(dataset, opt, pipe, dataset_path, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians.gaussian_init(dataset_path)
    
    scene = Scene(dataset, gaussians, dataset_path)
    scene.dataset_init()
    ...
    first_iter += 1
    SSIM_queue = collections.deque(maxlen=600)
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        try:
            spectrum, tx_pos, freq = next(scene.train_iter_dataset)
        ...
        f_opacity = gaussians.dist_signal_mapping(tx_pos, r_o, freq)
        ...
        render_pkg = render(viewpoint_cam, gaussians, pipe, bg, f_opacity, use_trained_exp=dataset.train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE)
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}",
                                        ...
                                        #   "Mean pixel error": f"{pixel_error:.6f}",
                                          ...
                                          "Median SSIM": f"{np.median(SSIM_queue):.6f}",
                                          })
            ...
            if iteration in testing_iterations:
                ...
                save_img_idx = 0
                all_ssim = []
                for test_input, test_label, freq in scene.test_iter: 
                    ...
                    render_pkg = render(viewpoint_cam, gaussians, pipe, bg, f_opacity)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[10000, 30000, 60000, 120000, 200000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[10000, 30000, 60000, 120000, 200000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument('--disable_viewer', action='store_true', default=False)
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[10000, 30000, 60000, 120000, 200000])
    parser.add_argument("--start_checkpoint", type=str, default = None)
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument("--dataset_path", type=str, default="./data/example1")
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.dataset_path, args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from)
