...
class MultiFrequency_Spectrum_dataset(Spectrum_dataset):

    def __init__(self, datadir, indexdir, freqdir):
        super().__init__(datadir, indexdir)
        self.dataset_freq = np.loadtxt(freqdir, dtype=float)

    def __getitem__(self, index):
        spectrum, tx_pos_i = super().__getitem__(index)
        freq_i = torch.tensor(self.dataset_freq[int(self.dataset_index[index]) - 1], dtype=torch.float32)
        return spectrum, tx_pos_i, freq_i

dataset_dict = {"rfid": Spectrum_dataset, "freq": MultiFrequency_Spectrum_dataset}
